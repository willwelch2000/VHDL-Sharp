...
/// </summary>
public abstract class Condition : ILogicallyCombinable<Condition, ConditionLogicStringOptions>
{
    /// <inheritdoc/>
    public IEnumerable<Condition> BaseObjects => [this];

    /// <inheritdoc/>
    public bool CanCombine(ILogicallyCombinable<Condition, ConditionLogicStringOptions> other)
    {
        return other.BaseObjects.First().Module == Module;
    }

    /// <inheritdoc/>
    public abstract string ToLogicString(ConditionLogicStringOptions options);
